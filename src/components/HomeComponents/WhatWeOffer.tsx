            ...
            data.map((item, i) => (
                <div  className= { `p-6 pt-8 rounded-md ${i === 0 ?"md:col-span-2":""}`}
                  style={{backgroundColor:item.bgColor,color:item.textColor} }>
                    <h2 className="text-28 md:text-30 font-medium whitespace-pre-line leading-tight">{item.title}</h2>
                    <p className=" text-14 font-bold pt-4" style={{opacity:i===0 ?1:0.7,fontWeight:i===0?"normal":"bold",fontSize:i==0 ?"18px":"14px" }}>{item.content}</p>
