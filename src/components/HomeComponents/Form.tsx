...
export const Form = () => {
    ...
    const setupRecaptcha = (): RecaptchaVerifier => {
        if (!window.recaptchaVerifier) {
            ...
            // Pass the container element, NOT a string
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const date = new Date();
            const formDataWithTime = { ...formData, Date: date.toLocaleDateString(), Time: date.toLocaleTimeString() };
        ...
        <div className="cont">
            <div className="bg-forest rounded-lg flex flex-col lg:flex-row md:flex-col pt-8 px-4 py-4">
                ...
                <div className="w-full lg:w-1/2 md:w-full md:px-32">
                    <form onSubmit={handleSubmit} className="flex flex-col justify-center">
                        <label className="text-white flex flex-col">Name:</label>
                        <input className="rounded p-2 text-black text-20" type="text" name="Name" value={formData.Name} onChange={handleChange} required />
                        <br />
                        <label className="text-white flex flex-col">Email:</label>
                        <input className="rounded p-2 text-black" type="email" name="Email" value={formData.Email} onChange={handleChange} required />
                        <br />
                        <div className="flex gap-4">
                            <label className="text-white flex flex-col w-[48%]">Board:
                                <input className="rounded p-2 text-black" type="text" name="Board" value={formData.Board} onChange={handleChange} />
                            </label>
                            <label className="text-white flex flex-col w-[48%]">Location:
                                <input className="rounded p-2 text-black" type="text" name="Location" value={formData.Location} onChange={handleChange} />
                            </label>
                        </div>
                        <br />
                        <label className="text-white flex flex-col">Phone:</label>
                        <input className="rounded p-2 text-black" type="text" name="Phone" value={formData.Phone} onChange={handleChange} />
                        <button type="button" className="bg-red-100 mt-4 p-2 rounded-[7px] text-black" onClick={sendOtp}>Send OTP</button>

                        {confirmationResult && (
                            <>
                                <input className="rounded p-2 text-black mt-2" type="text" placeholder="Enter OTP" value={otp} onChange={(e) => setOtp(e.target.value)} />
                                <button type="button" className="bg-green-100 mt-2 p-2 rounded-[7px] text-black" onClick={verifyOtp}>Verify OTP</button>
                            </>
                        )}
